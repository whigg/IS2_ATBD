...
for kk=1:numel(R)
        ...
        % run correction
        [D3, D3u, D3a]=correct_data(D2, params);
        % save output : calculate the median of each output parameter
        temp_med=median(D3.med-D3a.med);
        temp_centroid=median(D3.centroid-D3a.centroid);
        temp_med_uncorr=median(D3u.med-D3a.med);
        temp_centroid_uncorr=median(D3u.centroid-D3a.centroid);
        temp_N=median(D3.count);
        temp_N_uncorr=median(D3u.count);      
        ...
        temp_centroid=mean(D3.centroid-D3a.centroid);
        temp_med_uncorr=mean(D3u.med-D3a.med);
        temp_centroid_uncorr=mean(D3u.centroid-D3a.centroid);
        temp_N=mean(D3.count);
        temp_N_uncorr=mean(D3u.count);          
...
%-------------------------------------------------------
function [D3, D3u, D3a]=correct_data(D2, params)
...
for k=1:length(segs)-1;
    ...
    [D3.med(k), D3.centroid(k), D3.count(k), t_WF, N0, N_fpb_corr, D3.sigma_med(k), D3.sigma_centroid(k)]=fpb_corr_ATBD(D2sub.h, D2sub.channel, D2sub.pulse, params.N_channels,57, params.t_dead, false);
    [D3u.med(k), D3u.centroid(k), D3u.count(k), t_WF, N0, N_fpb_corr, D3u.sigma_med(k), D3u.sigma_centroid(k)]=fpb_corr_ATBD(D2sub.h, D2sub.channel, D2sub.pulse, params.N_channels, 57, params.t_dead, true);
